...
router.post("/employees", async (req, res, next) => {
  const { fullName, sex, maritalStatus, address, country, phoneNumber, emailAddress, passport, birthDate, employeeId, department, epfNumber, SOCSO, startDate, upload } = req.body;
  ...
  try {
    ...
    await db(`INSERT INTO employment_info(employeeId, department, epfNumber, socso, startDate, upload) VALUES ('${employeeId}', '${department}', '${epfNumber}', '${SOCSO}', '${startDate}', '${upload}');`);
...
router.put("/employees/:id", async function (req, res, next) {
  ...
  try {
    ...
    await db(`UPDATE employment_info SET employeeId = '${employeeId}', department = '${department}', epfNumber = '${epfNumber}', SOCSO = '${SOCSO}', startDate = '${startDate}', upload = '${upload}' WHERE id = ${id};`); 
    //get the updated list of employees
    let results = await db(employees);
    res.send(results.data);
  } catch (err) {
